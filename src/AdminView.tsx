...
export const AdminView: React.FC = () => {
  const { 
    ...
    paymentConfig, updatePaymentConfig, tables, toggleTableStatus 
  } = useApp();
  ...
  const stats = [
    { label: 'Total Sales', value: `₱${totalRevenue.toLocaleString()}`, icon: <DollarSign className="text-green-600" />, bg: 'bg-green-50' },
    { label: 'Total Orders', value: completedOrders.length, icon: <ShoppingBag className="text-blue-600" />, bg: 'bg-blue-50' },
    { label: 'Items Sold', value: totalItemsSold, icon: <Package className="text-purple-600" />, bg: 'bg-purple-50' },
    { label: 'Avg Order Value', value: `₱${completedOrders.length ? Math.round(totalRevenue / completedOrders.length) : 0}`, icon: <TrendingUp className="text-orange-600" />, bg: 'bg-orange-50' },
  ];
        ...
        <div className="container mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-4">
            <button 
              ...
              className="hidden md:flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-xl text-xs font-bold transition-all"
            >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                      ...
                      {completedOrders.slice(0, 10).map(order => (
                        <tr key={order.id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-6 py-4">
                            ...
                            <div className="text-[10px] text-gray-400">Table {order.tableNumber}</div>
                          </td>
                          <td className="px-6 py-4">
              ...
              <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6">
                ...
                <div className="space-y-4 mb-8">
                  <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Popular Items</h4>
                  <div className="space-y-2">
                    {popularItems.map((item) => (
                      <div key={item.name} className="flex justify-between items-center bg-gray-50 p-3 rounded-xl border border-gray-100">
                        ...
                        <span className="text-xs font-black text-indigo-600 bg-indigo-50 px-2 py-1 rounded-lg">{item.count} sold</span>
                      </div>
                    ))}
                  </div>
                </div>
                    ...
                    <div key={cat} className="flex flex-col items-center gap-2">
                      <div className="w-full flex-grow flex items-end justify-center min-h-[100px] gap-1">
                        <motion.div 
                          ...
                          className="w-10 bg-indigo-500 rounded-t-lg shadow-inner"
                        />
                      </div>
