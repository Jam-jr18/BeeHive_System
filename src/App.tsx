...
const LoginView: React.FC<{ onLogin: (role: ViewMode) => void, targetRole: ViewMode }> = ({ onLogin, targetRole }) => {
  const [password, setPassword] = useState('');
  const [error, setError] = useState(false);

  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    // Simple demo password: 'admin' for admin, 'staff' for staff
    const correctPass = targetRole === 'Admin' ? 'admin123' : 'staff123';
...
const AppContent: React.FC = () => {
  ...
  const renderContent = () => {
    ...
    if (authenticatedRole !== view) {
      return <LoginView onLogin={(role) => setAuthenticatedRole(role)} targetRole={view} />;
    }
  ...
  return (
    <div className="min-h-screen">
      ...
      <nav className="fixed top-4 left-4 z-50 group flex flex-col gap-2">
        {/* Invisible Trigger */}
