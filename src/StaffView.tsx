...
import { Clock, CheckCircle, ChefHat, Package, Trash2, User, Hash, Utensils, ShoppingBag } from 'lucide-react';
        ...
        <div className="container mx-auto flex justify-between items-center">
          ...
          <div className="flex gap-4 text-sm font-medium">
            <div className="flex items-center gap-1">
              ...
              <span>{activeOrders.length} Active Swarms</span>
            </div>
          </div>
        </div>
      </header>
              ...
              activeOrders.map(order => (
                <motion.div layout key={order.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
                  <div className="p-4 border-b bg-gray-50 flex justify-between items-start">
                    ...
                    <span className={cn("px-2 py-1 rounded-full text-[10px] font-bold border", getStatusColor(order.status))}>{order.status}</span>
                  </div>
